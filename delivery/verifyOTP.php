...
if (isset($_SESSION['otp_user_id'])) {
    ...
    echo "<script>alert('Session expired. Please log in again.'); window.location.href = 'LoginDelivery.php';</script>";
    exit();
}
...
if (isset($_POST['verify_otp'])) {
    ...
    $sql = "SELECT OTP_hash, OTP_expiry FROM delivery_admin WHERE del_id = ?";
    ...
    if ($user && hash('sha256', $otp) === $user['OTP_hash'] && time() < strtotime($user['OTP_expiry'])) {
        ...
        echo "<script>alert('Login successful. Redirecting to homepage.'); window.location.href = 'DashboardDelivery.php';</script>";
        exit();
    } else {
